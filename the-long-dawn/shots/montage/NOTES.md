...
| s3 `s3_ice.py` | 1580–1639 | ice | warm fire under a green sky | heightfield shore/promontory + tidewater glacier (ice cliffs, right) + art-directed moonlit massif (right) + low far shore (centre); aurora v2 = three explicit folded curtains (vertical sheets at ~100 km, Earth curvature) rendered per screen column by box-filtering curve segments (exact edge-on fold brightening, alias-free, full supersampled res; sharp lower edge, drifting rays, faint violet tops, Kasten–Young extinction); fjord = true mirror camera (terrain + sky + aurora) remapped through footprint-filtered ripples. Aurora code lives in `s3_ice.py` (the old `mt/sky.aurora_ray` is now unused) |
