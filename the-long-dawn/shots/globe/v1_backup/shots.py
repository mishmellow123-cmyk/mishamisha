...
class Dawn:
    ...
    def calm(self, t, H):
        ...
        if os.environ.get('LONGDAWN_NOCALM') == '1':      # the wordless cut (BIBLE_V2 cut B)
            return None
    ...
    def render_hdr(self, t, scale=1.0):
        ...
        web().draw(img, cam, 2600.0, t_anim=t, gain=0.9, scale=scale, calm=calm, sun=Sl, sparks=False,
                   ground_only=True)
        if os.environ.get('LONGDAWN_NO_HEARTHS') != '1':   # v2: no 'village hearths bloom across dark Africa'
        ...
        mask = (470.0, 560.0, 0.0 if os.environ.get('LONGDAWN_NOCALM') == '1' else 0.85 * max(ramp(t, 2255, 2268), 0.0))
