...
class Glyphs:
    ...
    def __init__(self, n=5200, n_ember=1300, seed=3):
        ...
        home[:, 1] += 2.0
        # 60% on the 'stage' in front of the lens
        stg = r.random(n) < 0.42
        ns = stg.sum()
        ds = rand_dirs(r, ns) * (r.random(ns) ** (1 / 3.0))[:, None]
        home[stg] = np.array([0.0, 2.8, -9.0]) + ds * np.array([18.0, 6.0, 11.0])
        # keep the lens clear: push non-heroes away from the camera corridor
        corr = np.array([0.6, 1.0, 8.2])
        dv = home - corr
        dd = np.linalg.norm(dv, axis=1)
        push = dd < 5.0
        home[push] = corr + dv[push] / dd[push, None] * (5.0 + 4.0 * r.random(push.sum()))[:, None]
        # other heroes: mid-stage, above the text band
        hz = r.uniform(-6.0, 1.0, nh)
        hx = r.uniform(-6.0, 6.0, nh)
        hy = r.uniform(2.5, 5.0, nh)
        ...
        s[ie] = r.lognormal(np.log(0.2), 0.18, n_ember)
        # ---- look
        ...
        self.sp_start = 394.0 + 8.0 * clamp01(rr / 30.0) + r.uniform(-3, 3, n)
    ...
    def state(self, t):
        ...
        if act.any():
            ...
            hh = h * (1 - smoothstep(0.0, 0.3, ua)) * (1 - ua)
...
CAM_A = [
    ...
    (412, (2.0, 7.0, 10.5), (0.0, -0.5, -1.0)),
    (432, (4.0, 12.0, 18.5), (0.0, -1.5, 0.0)),
...
HERO_PASSES = [
    ('火', 348, -0.42, 0.22, 0.36),
    ('\U0001D11E', 354, 0.46, 0.14, 0.36),
    ('كلمة', 360, -0.10, 0.30, 0.30),
    ('π', 366, 0.58, 0.28, 0.34),
    ('ज्ञान', 372, -0.56, 0.10, 0.28),
    ('A', 378, 0.20, 0.24, 0.34),
    ('ACGT', 384, -0.30, 0.32, 0.26),
    ('∞', 390, 0.52, 0.08, 0.34),
]
